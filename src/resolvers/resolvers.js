...
const resolvers = {
  ...
  Mutation: {
    signup: async (parent, args, context, info) => {
      ...
      //email ซ้ำไม่ควรสมัครได้
      const currentEmail = await User.find({});
      const isEmailExist =
        currentEmail.findIndex((user) => user.email === email) > -1;
      if (isEmailExist) {
      ...
      //2.ใช้ bcrypt เพื่อทำการเปลี่ยนแปลง password
